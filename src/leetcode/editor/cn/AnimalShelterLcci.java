...
class AnimalShelf {
    LinkedList<int[]> myShelf;
    public AnimalShelf() {
        myShelf=new LinkedList<>();
    }
    
    public void enqueue(int[] animal) {
        myShelf.addLast(animal);
    }
    
    public int[] dequeueAny() {
        if(myShelf.isEmpty()) return new int[]{-1,-1};
        return myShelf.removeFirst();
    }
    
    public int[] dequeueDog() {
        for(int[] animal:myShelf){
            if(animal[1]==1){
                myShelf.remove(animal);
                return animal;
            }
        }
        return new int[]{-1,-1};
