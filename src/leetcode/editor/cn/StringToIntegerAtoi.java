...
class Solution {
    public int myAtoi(String str) {
        Map[] states={
                ...
                new HashMap<Character,Integer>(){{put('d',2);}},
                new HashMap<Character,Integer>(){{put('d',2);}}
        };
        char t;
        int p=0;
    ...
    int trans(List<Character> characters){
        int sign=1;
        int start=1;
        if(characters.get(0)=='-') sign=-1;
        else if(characters.get(0)!='+') start=0;
        int ans=0;
        int bound=Integer.MAX_VALUE/10;
        int t=0;
        for(int i=start; i<characters.size(); ++i){
            t=characters.get(i)-'0';
            if(ans>bound||ans==bound&&t>7) return sign==1?Integer.MAX_VALUE:Integer.MIN_VALUE;
            ans*=10;
            ans+=t;
        }
        return sign*ans;
